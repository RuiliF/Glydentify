...
from train_add_residue_freeze_both import GTDonorDataset, get_criterion, eval_model
import torch
...
class Classifier(nn.Module):
    def __init__(self, checkpoint: str, num_labels: int, train_encoder: bool = False):
        super().__init__()
        self.encoder = EsmModel.from_pretrained(checkpoint)
        self.proj_dim = self.encoder.config.hidden_size
    ...
    def forward(self, input_ids, attention_mask):
        ...
        #x = output[:,0,:]
        # mean pooling
        aa_mask = (input_ids > 2).to(torch.float32).unsqueeze(-1)
        x = (output * aa_mask).sum(dim=1) / aa_mask.sum(dim=1)
        x = self.classifier(x)
        return x, output, aa_mask
    ...
    def from_pretrained(cls, checkpoint_path):
        ...
        model = cls(config["ckpt_name"], num_labels=len(label2id), train_encoder=train_encoder)
...
def train_model(df_train, df_val, label2id, criterion,
                ckpt_name="facebook/esm2_t33_650M_UR50D",
                lr=5e-5,
                weight_decay=1e-4,
                batch_size=12,
                epochs=10,
    ...
    train_dataset   = GTDonorDataset(df_train, "SaProt Sequence", 
                                     tokenizer, max_seq_len, 
                                     label2id, label_column="Donor"
                                     )
    val_dataset     = GTDonorDataset(df_val, "SaProt Sequence", 
                                     tokenizer, max_seq_len, 
                                     label2id, label_column="Donor"
                                     )
    ...
    dl_train = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, generator=generator)
    ...
    for i in tqdm(range(epochs), desc="Epoch", total=epochs):
        model.train()
        total_loss = 0.0
        for batch in tqdm(dl_train, desc="train", total=len(dl_train)):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sequence_checkpoint", type=str, default="westlake-repl/SaProt_650M_AF2")
    ...
    actual_max_len = df_tr["SaProt Sequence"].apply(len).max() // 2
    print(actual_max_len)
    ...
    checkpoint_dir = f"checkpoints/mlp-{'freeze_encoder' if not args.train_encoder else 'train_encoder'}_domain/{args.fold}/" + f"{args.sequence_checkpoint.split('/')[-1]}_" + timestamp
    os.makedirs(checkpoint_dir, exist_ok=True)
    ...
    wandb.init(project="saprot_unimol", name=f"[domain] {'gta' if 'gta' in args.fold else 'gtb'} mlp {'freeze_encoder' if not args.train_encoder else 'train_encoder'}" + timestamp, config=config)
